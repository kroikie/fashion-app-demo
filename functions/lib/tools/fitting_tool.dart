...
final fittingTool = ai.defineTool(
  ...
  fn: (input, context) async {
    ...
    final parts = <gai.Part>[
      gai.TextPart(toolInstructions),
    ];

    for (var i = 0; i < accessories.length; i++) {
      final bytes = await fetchImageBytes(accessories[i]);
      parts.add(gai.TextPart('Product image ${i + 1} to apply to the person below:'));
      parts.add(gai.DataPart('image/png', bytes));
    }

    final userBytes = await fetchImageBytes(userImageUri);
    parts.add(gai.TextPart('=== IDENTITY ANCHOR ===\n'
    ...
    parts.add(gai.DataPart('image/jpeg', userBytes));
    ...
    gai.DataPart? genPart;
    for (final part in candidate.content.parts) {
      if (part is gai.DataPart) {
        genPart = part;
        break;
      }
    }
    ...
    final outBytes = genPart.bytes;
    final storageBucket = Platform.environment['GCS_BUCKET'] ?? 'flutter-firebase-fashion.firebasestorage.app';
